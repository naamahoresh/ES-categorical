...
class CatES:
    ...
    def test_exp(self, numThread):
        ...
        for i in range(1, self.niter + 1):  # num iterations
            if self.f_function_counter > 0:  # num calls to the F function
                ...
                if self.withSurr:  # exp with surrogate
                    if (i < self.intiNumIterWituoutSurr or i % 9 == 0 or i % 10 == 0):
                        ...
                        if self.sourrogateTyp == "RBFN":
                            F = surrogate.surrogateRBFN2(self.surr_Xarchive[:, 0:len(self.surr_Farchive)],
                                                        self.surr_Farchive,
                                                        Xnew[0:self.N, :], self.fileLocSave_bythread,
                ...
                if (np.mod(i, int(self.niter / 10)) == 0 or i < 1 or self.niter % 1 == 0):
    ...
    def init_Exp(self):
        ...
        X = self.initCES(pmut, self.typeExp)
    ...
    def initCES(self, pmut, typeInti):
        X = np.zeros((self.N, self.popSize), dtype=np.int)
        if (typeInti == 1):  # create random data
            X=self.initbinary()
        ...
        pmut_vector = pmut * np.ones(self.popSize)  # individual mutation rates
    ...
    def varCES(self, X, F, ARCH):
        ...
        num_elitism = 2
        lowest_pmut = 0.005
        lowest_pmut_defult = 0.01
        tau = 1 / (math.sqrt(self.N))

        elitism_vec = self.elitism(X,F,num_elitism)
        Xn=self.tourment(X, F, Tournament_size, mu)
        Xne=self.cross_over(Xn,mu,pCrossOver)
        Xnew,tmp=self.mutation(Xne,F, ARCH,mu,lowest_pmut,lowest_pmut_defult,tau)

        ARCH = np.hstack((ARCH, tmp))
        Xnew[:, 0:num_elitism] = elitism_vec

        return Xnew, ARCH


    def tourment(self,X,F,Tournament_size,mu):
        Xn = np.zeros((self.N + 1, mu))
        for i in range(0, mu):
            parents = np.ceil(
                mu * np.random.rand(Tournament_size))  # choose random competitors from the parents generation
    ...
    def elitism(self,X,F,num_elitism):
        _, elitism_index = np.unique(F, return_index=True)
        if (len(elitism_index) < num_elitism):
            num_elitism = len(elitism_index)
        if (self.opt == "max"):
            elitism_index = elitism_index[(-1 * num_elitism):]
        else:
            elitism_index = elitism_index[:num_elitism]
        elitism_vec = X[:, elitism_index]
        return elitism_vec


    def cross_over(self,Xn,mu,pCrossOver):
        Xne = np.zeros((self.N + 1, mu))
        for i in range(int(mu / 2)):

            if (random.random() < pCrossOver):  # do crossover
    ...
    def mutation(self,Xne,F, ARCH,mu,lowest_pmut,lowest_pmut_defult,tau):
        Xnew = np.full((self.N + 1, mu), -1, dtype=float).squeeze()
        for i in range(mu):
            Xnew[self.N, i] = (1 / (1 + ((1 - Xne[self.N, i]) / (Xne[self.N, i])) * (np.exp(-(tau) * (np.random.normal())))))
            if (Xnew[self.N, i] < lowest_pmut):
                Xnew[self.N, i] = lowest_pmut_defult
    ...
    def CES_Mutate(self, Xold, pmut, f):
        Xnew = np.zeros(self.N)
        for i in range(self.N):
            if (random.random() < pmut):  # we do the mutation in a pmut probability (mutation probability)
    ...
    # mutate the velues
    def CES_Mutate_combinatoric(self, Xold, pmut, f):
    ...
    for problem in problem_list:
        for sourrogate in sourrogate_list:
            ...
            for seed in range(numToCompare):
                ...
                cat = CatES(fitness, DEVector, typeExp, numPar, popSize, seed, max_attainable, niter, opt, fixNumber,sourrogate,
                ...
                with open("/home/naamah/Documents/CatES/result_All/" + problem + "/" + fileName + "/General_Info.txt",
                    ...
                    file.write("Num of iterations before using the surrogate model (for each Thread): {}".format(intiNumIterWituoutSurr) + "\n")
                    if sourrogate=="RBFN":
                        file.write("The centers were selected with the help Kmeans algorithem: {}".format(isKmeans) + "\n")
