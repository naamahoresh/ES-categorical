...
class CatES:
    def __init__(self, fitnessfun, DEVector, typeExp, numPar, popSize, seed=0, max_attainable=np.inf, niter=100,
                 opt='max', repeated_Number=False, sourrogate="KRIGING", saveNum=0,f_function_counter=600, withSurr=True, problem="QAP",
                 pmut=0.4, lowest_pmut= 0.005, lowest_pmut_defult= 0.01, pCrossOver = 0.5,Tournament_size=10,num_elitism=2, XInit=None):
        ...
        self.fileLocSave_general="/home/naamah/Documents/CatES/result_All/"+problem+"/"+sourrogate+"_"+str(saveNum)
        ...
        with open(self.fileLocSave_general + "/ES_configuration.txt",'a') as file:
            file.write("Thread num: {}\n pmut = {}\n lowest_pmut = {}\n lowest_pmut_defult = {}\n pCrossOver = {}\n Tournament_size = {}\n num_elitism = {}\n\n\n".format(str(seed + 1), pmut, lowest_pmut, lowest_pmut_defult, pCrossOver, Tournament_size, num_elitism))
        file.close()
            ...
            if self.f_function_counter>0: #num calls to the F function
                ...
                if self.withSurr: #exp with surrogate
                    if (i<20 or i % 9 == 0 or i % 10 == 0):
                        ...
                        F =surrogate.surrogateKriging(self.surr_Xarchive[:, 0:len(self.surr_Farchive)+1], self.surr_Farchive,
                                                        Xnew[0:self.N, :],self.fileLocSave_bythread, self.fileLocSave_general, self.withArchive,True,self.problem)  # call to the soraggate model
                        ...
                        if self.sourrogateTyp=="RBFN":
                            F = surrogate.surrogateRBFN(self.surr_Xarchive[:, 0:len(self.surr_Farchive)], self.surr_Farchive,
                                                        Xnew[0:self.N, :],self.fileLocSave_bythread,self.fileLocSave_general, self.withArchive, False, self.problem)  # call to the soraggate model
                ...
                if (np.mod(i, int(self.niter / 50)) == 0 or i<1 or self.niter%1==0):
                    stri="Thread number: {} - evals: {} - max fitness in the generation = {}".format(numThread + 1, i,fopt[-1])
                    ###print(stri)
    ...
    def varCES(self, X, F, ARCH):
        ...
        for i in range(int(mu / 2)):
            ...
            if (random.random() < self.pCrossOver): #do crossover
                ...
                if self.repeated_Number:
                    col_tmp = np.hstack((Xn[0:loc, int(2 * i) + 1], Xn[loc:self.N, int(2 * i)], [Xn[self.N, int(2 * i) + 1]]))
                    Xne[:, int((2 * i))] = col_tmp
                    col_tmp2 = np.hstack((Xn[0:loc, int(2 * i)], Xn[loc:self.N, int(2 * i) + 1], [Xn[self.N, int(2 * i)]]))
...
class InitProblem():
    ...
    def QAP(self):
        ...
        DEVector = [1]  # number of levels for each dimnation. the index 0 will represent all the dimnation have the same lav el (0=same lavel, 1= different)
...
def main(pmut=0.4, lowest_pmut= 0.005, lowest_pmut_defult= 0.01, pCrossOver = 0.5, Tournament_size=10, num_elitism=2, problems_list="NaN",
         sourrogates_list="NaN", FileNumToSave=100, numThreads=2, XInit=None,niter=70):
    pmut_org = pmut; lowest_pmut_org = lowest_pmut;lowest_pmut_defult_org = lowest_pmut_defult; pCrossOver_org = pCrossOver; Tournament_size_org = Tournament_size; num_elitism_org = num_elitism;
    ...
    if sourrogates_list == "NaN":
        sourrogate_list = ["KNN"]  # RBFN KRIGING "RF","SVM","KNN"
    ...
    for problem in problem_list:
        for sourrogate in sourrogate_list:
            ...
            for seed in range(numToCompare):
                ...
                if seed==1:
                    withSurr=False
                    pmut = 0.4; lowest_pmut = 0.005; lowest_pmut_defult = 0.01; pCrossOver = 0.5; Tournament_size = 10; num_elitism = 2;
                else:
                    pmut= pmut_org; lowest_pmut=lowest_pmut_org; lowest_pmut_defult = lowest_pmut_defult_org; pCrossOver = pCrossOver_org; Tournament_size = Tournament_size_org; num_elitism = num_elitism_org;


                cat = CatES(fitness, DEVector, typeExp, numPar, popSize, seed, max_attainable, niter, opt, repeated_Number,sourrogate,
                            FileNumToSave,call_F_function_counter, withSurr, problem,pmut, lowest_pmut, lowest_pmut_defult, pCrossOver,Tournament_size,num_elitism,XInit)
                ...
                if withThreads:
                    fmax.append(Parallel(n_jobs=numThreads, verbose=0)(delayed(cat.test_exp)(i)for i in range (numThreads)))
                else:
                ...
                #general details
                with open( "/home/naamah/Documents/CatES/result_All/"+problem+"/"+fileName+"/General_Info.txt", 'a') as file:
                    if withSurr:
...
if __name__ == '__main__':
    ...
    numThreads = 30
    niter = 2100
    FileNumToSave = 100123

    print("main ans: {}".format(main(pmut=0.1, lowest_pmut= 0.09, lowest_pmut_defult= 0.02, pCrossOver = 0.34, Tournament_size=14, num_elitism=4,
                                        FileNumToSave=FileNumToSave, numThreads=numThreads, XInit=None, niter=niter))) # for Pump
